...
parser.add_argument("--loglevel", "-l",
                    help="Verbosity of the logging screen prints [D=Debug, I=Info, W=Warning, E=Error, and C=Critical (Default)]")
...
if args.loglevel:
    if args.loglevel.upper() == 'D':
        ...
        logging.basicConfig(
            format='[%(filename)s:%(lineno)d] - %(message)s', level=logging.INFO)
    elif args.loglevel.upper() == 'W':
        logging.basicConfig(
            format='[%(filename)s:%(lineno)d] - %(message)s', level=logging.WARNING)
    elif args.loglevel.upper() == 'E':
        logging.basicConfig(
            format='[%(filename)s:%(lineno)d] - %(message)s', level=logging.ERROR)
    elif args.loglevel.upper() == 'C':
        logging.basicConfig(
            format='[%(filename)s:%(lineno)d] - %(message)s', level=logging.CRITICAL)
...
#     file_dir = os.getcwd() + '/static/data/' + cur_proj.name + \
#         '/' + cur_proj.current_dataset.name + '.dat'
...
#     x = threading.Thread(target=create_thumbnail_and_preview,
#                          args=(project_name, "mp4",))
#     x.daemon = False
#     x.start()
...
#             os.getcwd() + "/static/data/" + project_name + \
...
#         # PORTRAIT
#         if height > width:
#             cmd = 'ffmpeg -i ' + root_data_path + project_name + '/' + project_name + '.' + extension + \
#                 ' -filter:v scale=200:-1 ' + root_data_path + \
#                 project_name + '/temp_' + project_name + '.' + extension

#         # LANDSACPE
#         else:
#             cmd = 'ffmpeg -i ' + root_data_path + project_name + '/' + project_name + '.' + extension + \
#                 ' -filter:v scale=202:-1 ' + root_data_path + \
#                 project_name + '/temp_' + project_name + '.' + extension
...
#         cmd = 'ffmpeg -i ' + root_data_path + project_name + '/' + project_name + '.' + extension + \
#             ' -vf "setpts=N/TB/1000" -r 2 -loop 0 ' + root_data_path + \
#             project_name + '/temp_' + project_name + '_thumbnail.gif'
#         results = os.popen(cmd).read()
...
#             # Debugging only
#             logging.debug('Project Creation Request: ' + project_name +
...
#                     # Create Thumbnail and Preview in a different thread
#                     x = threading.Thread(target=create_thumbnail_and_preview, args=(
#                         project_name, vidfile[-3:],))
#                     x.daemon = False
#                     x.start()
...
#     cmd = "ffprobe -v quiet -print_format json -show_streams " + \
#         os.getcwd() + "/static/data/" + video_name + "/" + video_name + '.mp4'
#     results = os.popen(cmd).read()
#     metadata = json.loads(results)
#     # print(results)
...
#     success, message = PM.user_config.save_default_camera(
#         PM.current_project.name, angle)
...
#     success1, message1 = PM.current_project.current_dataset.track_rename(
#         old_name, new_name)

#     # TODO: Should only save when dataset is saved
#     success2, message2 = PM.current_project.current_dataset.save_dataset(
#         force=True)
...
#     success1, message1 = PM.current_project.current_dataset.track_change_color(
#         track_name, new_color)
...
#     success1, message1 = PM.current_project.current_dataset.track_change_number(
#         track_name, new_number)
...
#     logging.info('writeNewTrack %s %s %s', track_name,
#                  track_number, track_color)
#     success1, message1 = PM.current_project.current_dataset.track_add_new(
#         track_name, track_number, track_color)
...
#     success1, message1 = PM.current_project.current_dataset.track_detele(
#         type_name, False)
...
#     success, message = PM.current_project.current_dataset.save_dataset(
#         all_data)
...
#     success, message = PM.current_project.current_dataset.save_as_dataset(
#         name, all_data)
